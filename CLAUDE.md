...
- **`.category-shine`** — per-entry shine for `TimelineEntry`'s "See details →" link. Reuses the brand-shine sweep but the underlying solid color is driven by an inline `--shine-color` CSS variable, so the link inherits the entry-type color (orange for projects, blue for education, green for experience).
...
- **Reduced-motion contract** — every shine/animation class above is listed in a single `@media (prefers-reduced-motion: reduce) { animation: none }` block. Add new shimmer classes there too.
...
**`src/data/timeline.js`** — Array of timeline entries (`id`, `type`, `title`, `subtitle`, `period`, `startYear`, `startMonth`, `endYear`, `endMonth`, `description`, optional `highlights`, `link`, `expanded`, `liveUrl`, `previewImage`, `subtitleLogo`) + a `timelineTypes` map (education/experience/project, each with color/bg/label/icon). Type colors are reused from `skillColors.js` to keep the palette coherent.

- `endYear`/`endMonth` drive horizontal-mode positioning and sort order. `endMonth` accepts fractional values (e.g., `10.66`) — cards are nudged off their real end date to avoid overlapping each other or the year ticks. The `period` string always shows the true date. The percentage you read in the `endMonth + 0.5` math is the card's **left edge** position, not its visual center (no `translateX(-50%)` on `.timeline-entry--horizontal`); to land a target left-edge percent, solve `endMonth = (target/100 * 13 + 7) - 0.5`.
...
_Completed: resume interactive timeline, Hero + timeline scroll animations, animated aurora background, wave-based meteor shower, Formspree contact form, Firestore-backed guestbook + cross-device project upvotes (real-time via `onSnapshot`), metallic-blue subheading + shine variants, Open Graph thumbnail._
