...
│   ├── components/
│   │   ├── Navbar.jsx         # Sticky navbar + dark mode toggle + smooth scroll links + neon-lamp active-section glow
...
- **`.nav-indicator` + `::before` (neon-lamp)** — the active-section underline now carries a trapezoid glow above it via `::before` with `clip-path: polygon(0% 0%, 100% 0%, 65.625% 100%, 34.375% 100%)`. Combined with `width: 160%; transform: translateX(-50%)` this anchors the cone's bottom corners at exactly 25% / 75% of the indicator's width (the `\` and `/` slash edges) and flares wider above. A radial gradient (white-tinted core → bright color → halo) fills the cone; `filter: blur(2.5px)` softens the falloff. Light mode: violet (`#a78bfa` family). Dark mode: emerald (`#10b981` family). `@keyframes neon-glow-pulse` oscillates opacity `0.78 ↔ 1` over 2.4s for a glow-stick breathing effect.
- **`.nav-link-lit`** — applied by `Navbar.jsx` to whichever Nav.Link matches `targetSection` (the same value driving the indicator's position, including hover states). Adds a layered `text-shadow` (4px tight + 12–14px halo) in the matching color so the active link reads as physically lit by the lamp below. Nav links carry `position: relative; z-index: 1` so their text paints above the trapezoid glow.
- **Reduced-motion contract** — every shine/animation class above is listed in `@media (prefers-reduced-motion: reduce) { animation: none }` blocks (search the file). `pill-shine`, `neon-glow-pulse`, and the `text-shadow` transition on `.nav-link-lit` all honour this. Add new shimmer classes there too.
...
- **Active nav section** — `useState` (`activeSection`) + `useState` (`hoveredSection`) in `Navbar.jsx`. `targetSection = hoveredSection ?? activeSection` drives both the `.nav-indicator` position (via `useLayoutEffect` measuring the active link's bounding rect) AND the `.nav-link-lit` className applied to the matching Nav.Link. Scroll-spy `IntersectionObserver` on `/` keeps `activeSection` in sync as the user scrolls Home; route changes set it directly via `useEffect`.
...
- `entry.certificateUrl` renders the `.certificate-pill` ("View certificate" + `FaFileAlt`) at the bottom-right of the card. When `collapseHighlights` is also set, the pill shares a `justify-content-between` row with the See more / less toggle. Otherwise it renders in its own right-aligned row at the bottom.
